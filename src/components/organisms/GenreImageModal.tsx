...
export function GenreImageModal({ genreName, onClose }: Props) {
  ...
  async function handleApply() {
    ...
    img.onload = async () => {
      ...
      try {
        await invoke("set_genre_image_from_base64", { genreName, imageBase64: base64 });
        queryClient.invalidateQueries({ queryKey: ["genre-image", genreName] });
        onClose();
      } catch (err) {
        setError(String(err));
        setSaving(false);
      ...
      >
        <div className="flex items-start justify-between mb-5">
          <div className="min-w-0 pr-3">
            <h3 className="text-base text-[#f0ead8]" style={{ fontFamily: "Fraunces, serif" }}>Set Genre Image</h3>
            <p className="text-[10px] text-[#3a3628] font-mono mt-0.5 truncate">{genreName}</p>
          </div>
        ...
        {!imgSrc ? (
          ...
          <>
            ...
            {/* Change file */}
            <label className="inline-block mt-3 text-[10px] font-mono text-[#3a3628] hover:text-[#7a7060] cursor-pointer transition-colors">
              <input type="file" accept="image/*" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) loadFile(f); }} />
