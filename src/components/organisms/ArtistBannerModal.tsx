...
export function ArtistBannerModal({ artistName, onClose }: Props) {
  ...
  // minZoom fills the 3:1 crop box (short side wins)
  ...
  async function handleApply() {
    ...
    canvas.width = 1920; canvas.height = 640;
    ...
    img.onload = async () => {
      ...
      ctx.drawImage(img, srcX, srcY, srcW, srcH, 0, 0, 1920, 640);
      const base64 = canvas.toDataURL("image/jpeg", 0.92).split(",")[1];
      try {
        await invoke("set_artist_banner_from_base64", { artistName, imageBase64: base64 });
        queryClient.invalidateQueries({ queryKey: ["artist-banner", artistName] });
        queryClient.invalidateQueries({ queryKey: ["artist-banner-custom", artistName] });
        onClose();
      } catch (err) {
        setError(String(err));
        setSaving(false);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-base text-[#f0ead8]" style={{ fontFamily: "Fraunces, serif" }}>Set Banner</h3>
        ...
        {!imgSrc ? (
          ...
          >
            ...
            <span className="text-[10px] text-[#3a3628] font-mono">Wide landscape recommended · 3:1 ratio</span>
